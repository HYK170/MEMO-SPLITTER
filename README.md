...
2. **INPUT** — 분할할 원본 파일
3. **Multimedia 폴더** — `저장된 파일 이름` 컬럼의 상대 경로 기준 루트
4. **SPLIT 실행** 클릭
...
- 출력 HTML은 셀 마크업을 **그대로** 복사 (이미지 임베드 없음)
...
- `저장된 파일 이름` — Multimedia 하위 상대 경로. 여러 개는 줄바꿈(`<br>` 포함)으로 구분
...
- HTML: 파일 복사만, 셀 HTML(`<img>` 등)은 원본 그대로 유지
